...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    //Filtrer les produits qui sont dans un intervalle de prix et qui appartiennent a une categorie bien defien
    @Query(value = "SELECT * FROM product WHERE (price BETWEEN :min AND :max) AND category = :category", nativeQuery = true)
    Page<Product> filterByCategoryAndPrice(@Param("min") BigDecimal min, @Param("max") BigDecimal max, @Param("category") String category, Pageable pageable);

    Page<Product> findByCategory(String category, Pageable pageable); //Pageable: l'ordre dans lequel on va veut trier et le nombre d'elements a afficher sur la page
}
